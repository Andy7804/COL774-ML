...
from modules.vqa_modelBERT import VQAModelBERT
...
def train(args):
    ...
    model = VQAModelBERT(vocab_size=30522, num_classes=num_classes)
    model.to(device)

    # If model_path is provided, load weights from the best previous model
    if args.model_path:
        print(f"Loading model weights from {args.model_path}")
        checkpoint = torch.load(args.model_path, map_location=device)
        # Load the weights for components other than the text encoder
        # This is important since we've changed the text encoder architecture
        ...
        # Initialize image encoder from checkpoint
        model.image_encoder.load_state_dict({k.replace('image_encoder.', ''): v 
                                           for k, v in checkpoint['model_state_dict'].items() 
                                           if 'image_encoder' in k})
        
        # Keep image encoder frozen or not based on part 9
        for param in model.image_encoder.resnet_backbone.parameters():
            param.requires_grad = True  # Keep frozen by default, change if needed
...
def train2(args):
    ...
    best_val_f1 = checkpoint['val_metrics']['f1_score'] if 'val_metrics' in checkpoint else 0.0
    
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=2, verbose=True)
    criterion = nn.CrossEntropyLoss()
    
    # For tracking metrics
    num_epochs = 10  # Additional epochs to train
    train_losses, val_losses = [], []
    train_accs, val_accs = [], []
...
def inference(args):
    ...
    model = VQAModelBERT(vocab_size=vocab_size, num_classes=num_classes)
    checkpoint = torch.load(args.model_path, map_location=device)
    model.load_state_dict(checkpoint['model_state_dict'])
    model.to(device)
    model.eval()
